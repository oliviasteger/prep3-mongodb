...
export default class UserConcept {
  ...
  async getById(_id: ObjectId) {
    const _user = await this.users.readOne({ _id });
    if (!_user) {
      throw new NotFoundError(`User with ID ${_id} not found!`);
    }
    return { msg: "User found successfully!", user: _user };
  }
  ...
  async update(_id: ObjectId, update: Partial<UserDoc>) {
    const _user = await this.users.updateOne({ _id }, update);
    return { msg: "User updated successfully!", user: _user };
  }
